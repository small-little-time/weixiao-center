      ...
      >
        <div v-for="item in list" :key="item.id" style="background: white;">
          <div style="margin: 3px;padding-top: 2px;">
            <van-tag mark v-show="item.state === 1" type="warning" style="float: right;">审核中</van-tag>
            <van-tag mark v-show="item.state === 2" type="success" style="float: right;">通过</van-tag>
            <van-tag mark v-show="item.state === 3" type="danger" style="float: right;">不通过</van-tag>
          ...
          <div style="background: white;text-align: right;padding-right: 15px;">
            ...
            <van-icon name="close" @click="cancelActivity(item.id)"/>
          </div>
...
import { getMyPublish, cancelMyPublish } from './api/myActivity'
  ...
  methods: {
    ...
    cancelActivity (id) {
      Dialog.confirm({
        ...
        cancelMyPublish(params).then(res => {
        })
      }).catch(() => {
        // on cancel
      })
